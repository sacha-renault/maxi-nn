...
{
    inline bool areShapeEqual(const std::vector<int>& s1, const std::vector<int>& s2) {
        if (s1.size() != s2.size()) {
            return false;
        } else {
    ...
    inline bool areShapeBroadcastable(const std::vector<int>& s1, const std::vector<int>& s2) {
        int len1 = s1.size();
        int len2 = s2.size();
    ...
    inline std::vector<int> calculateBroadcastedShape(const std::vector<int>& s1, const std::vector<int>& s2) {
        int i = s1.size() - 1;
        int j = s2.size() - 1;
...
Eigen::Matrix<T, -1, -1> broadcastView(const Eigen::Matrix<T, -1, -1>& data,
                                        const std::vector<int>& current_shape,
